...
public:
    ...
    int calheight(Tree *root)
    {
        ...
        if (root != NULL)
        {
            xheight = calheight(root->leftNode); // recursive function for cal height--->cross edge
            yheight = calheight(root->rightNode);
            if (xheight > yheight)
                return xheight + 1;
            else
                return yheight + 1;
    ...
    void LevelOrder(Tree *root)
    {
        ...
        while (!q.empty())
        {
            Tree *tempPtr = q.front();
            cout << tempPtr->data << " ";
            q.pop();
            if (tempPtr->leftNode != NULL)
                q.push(tempPtr->leftNode);
            if (tempPtr->rightNode != NULL)
                q.push(tempPtr->rightNode);
        }
    }
};
...
int main()
{
    ...
    // cout << "\n Total height of the tree:\t" << tree.calheight(&tree);
}
